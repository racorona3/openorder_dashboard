...
<style>
  :root {{
    --bg:#090d14;--surface:#0d1420;--surface2:#111c2e;--border:#1a2d4a;
    --accent:#1565c0;--accent2:#1e88e5;--glow:#1e88e540;
    --text:#cdd8e8;--muted:#5a7a9a;
  ...
  *,*::before,*::after{{box-sizing:border-box;margin:0;padding:0}}
  ...
  .search-label{{font-family:var(--mono);font-size:11px;letter-spacing:.15em;color:var(--muted);text-transform:uppercase;margin-bottom:10px;display:block}}
  ...
  .result-card.sa-priced{{border-color:#00e67630}}.result-card.sa-priced::before{{background:var(--green)}}
  .result-card.ga-priced{{border-color:#ce93d830}}.result-card.ga-priced::before{{background:var(--purple)}}
  .result-card.unpriced{{border-color:#ffab0030}}.result-card.unpriced::before{{background:var(--amber)}}
  .result-card.off-contract{{border-color:#ff525230}}.result-card.off-contract::before{{background:var(--red)}}
  .rc-header{{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px}}
  .rc-pn{{font-family:var(--mono);font-size:20px;font-weight:600;color:#fff;letter-spacing:.04em}}
  .badge{{font-family:var(--mono);font-size:10px;letter-spacing:.12em;padding:4px 10px;border-radius:20px;text-transform:uppercase;font-weight:600}}
  ...
  .rc-field .fk{{font-family:var(--mono);font-size:9px;letter-spacing:.15em;text-transform:uppercase;color:var(--muted)}}
  .rc-field .fv{{font-family:var(--mono);font-size:14px;color:#e8f0fe}}
  .rc-field .fv.price{{font-size:22px;font-weight:700;color:var(--green)}}
  .rc-field .fv.amber{{color:var(--amber)}}.rc-field .fv.muted{{color:var(--muted);font-style:italic}}
  .rc-message{{font-size:13px;color:var(--text);line-height:1.6;margin-top:4px}}
...
function buildCard(pn){{
  ...
  if(sa){{
    ...
    <div class="rc-field"><span class="fk">Action</span><span class="fv amber">Submit RFQ to Honeywell</span></div>`;
  }}else{{
